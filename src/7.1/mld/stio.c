...
extern void st_error(char* s, ...);
extern void st_internal(char* s, ...);
extern void st_warning(char* s, ...);
extern int ldfsymorder(pHDRR phdr, long symptr);
static int st_read(int fd, int ordered, int offset, char* ptr, int size);
extern void swap_dn(pDNR pdn, long count, long destsex);
extern void bzero(void*, unsigned int);

short stmagic = magicSym;
...

{
    ...
    if (hdr.magic != magicSym && hdr.magic != magicSym_cia) {
        if (hdr.magic == swap_half(magicSym) || hdr.magic == swap_half(magicSym_cia)) {
