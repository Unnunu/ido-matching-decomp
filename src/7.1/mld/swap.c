...
long destsex;
{
    ...
    for(i = 0 ; i < count ; i++){
        ...
        pext[i].weakext = sextr.u.fields.weakext;
        } else {
        ...
        sextr.u.fields.weakext = pext[i].weakext;
...
long destsex;
{
    ...
    for(i = 0 ; i < count ; i++){
        ...
        prpd[i].irpss          = swap_word(prpd[i].irpss);
...
void
swap_dynamic(pdyn, destsex)
register Elf32_Dyn *pdyn;
long destsex;
{
    pdyn->d_tag = swap_word(pdyn->d_tag);
    pdyn->d_un.d_val = swap_word(pdyn->d_un.d_val);
    pdyn->d_un.d_ptr = swap_word(pdyn->d_un.d_ptr); // BUG
}

void
swap_rel_dyn(prel, destsex)
register Elf32_Rel *prel;
long destsex;
{
    prel->r_offset = swap_word(prel->r_offset);
    prel->r_info = swap_word(prel->r_info);
}

void
swap_reginfo(preg, destsex)
register Elf32_RegInfo *preg;
long destsex;
{
    preg->ri_gprmask = swap_word(preg->ri_gprmask);
    preg->ri_cprmask[0] = swap_word(preg->ri_cprmask[0]);
    preg->ri_cprmask[1] = swap_word(preg->ri_cprmask[1]);
    preg->ri_cprmask[2] = swap_word(preg->ri_cprmask[2]);
    preg->ri_cprmask[3] = swap_word(preg->ri_cprmask[3]);
    preg->ri_gp_value = swap_word(preg->ri_gp_value);
}

void
swap_dynsym(psym, destsex)
register Elf32_Sym *psym;
long destsex;
{
    psym->st_name = swap_word(psym->st_name);
    psym->st_value = swap_word(psym->st_value);
    psym->st_size = swap_word(psym->st_size);
    psym->st_shndx = swap_half(psym->st_shndx);
}

void
swap_conflict(pconf, destsex)
register Elf32_Conflict *pconf;
long destsex;
{
    *pconf = swap_word(*pconf);
}

void
swap_got(pgot, destsex)
register Elf32_Got *pgot;
long destsex;
{
    pgot->g_index = swap_word(pgot->g_index);
}

void
swap_liblist(plib, destsex)
register Elf32_Lib *plib;
long destsex;
{
    plib->l_name = swap_word(plib->l_name);
    plib->l_time_stamp = swap_word(plib->l_time_stamp);
    plib->l_checksum = swap_word(plib->l_checksum);
    plib->l_version = swap_word(plib->l_version);
    plib->l_flags = swap_word(plib->l_flags);
}

void
swap_msym(pmsym, destsex)
register Elf32_Msym *pmsym;
long destsex;
{
    pmsym->ms_hash_value = swap_word(pmsym->ms_hash_value);
    pmsym->ms_info = swap_word(pmsym->ms_info);
}
...
{
    ehdr->e_type = swap_half(ehdr->e_type);
    ehdr->e_machine = swap_half(ehdr->e_machine);
    ehdr->e_version = swap_word(ehdr->e_version);
    ehdr->e_entry = swap_word(ehdr->e_entry);
    ehdr->e_phoff = swap_word(ehdr->e_phoff);
    ehdr->e_shoff = swap_word(ehdr->e_shoff);
    ehdr->e_flags = swap_word(ehdr->e_flags);
    ehdr->e_ehsize = swap_half(ehdr->e_ehsize);
    ehdr->e_phentsize = swap_half(ehdr->e_phentsize);
    ehdr->e_phnum = swap_half(ehdr->e_phnum);
    ehdr->e_shentsize = swap_half(ehdr->e_shentsize);
    ehdr->e_shnum = swap_half(ehdr->e_shnum);
    ehdr->e_shstrndx = swap_half(ehdr->e_shstrndx);
}
...
{
    phdr->p_type = swap_word(phdr->p_type);
    phdr->p_offset = swap_word(phdr->p_offset);
    phdr->p_vaddr = swap_word(phdr->p_vaddr);
    phdr->p_paddr = swap_word(phdr->p_paddr);
    phdr->p_filesz = swap_word(phdr->p_filesz);
    phdr->p_memsz = swap_word(phdr->p_memsz);
    phdr->p_flags = swap_word(phdr->p_flags);
    phdr->p_align = swap_word(phdr->p_align);
}
