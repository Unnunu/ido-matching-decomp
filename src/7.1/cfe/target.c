        ...
        case Plus_expr:
            if (IS_SIGNED_TYPE(type)) {
                if (IS_64BIT_TYPE(type)) {
                    return SAME_SIGNS(val1, val2) && 
                            DIFFERENT_SIGNS(val1 + val2, val2);
                } else {
                    return SAME_SIGNS((int)ICONSTANT(op1).value, (int)ICONSTANT(op2).value) && 
                            DIFFERENT_SIGNS((int)ICONSTANT(op1).value + (int)ICONSTANT(op2).value, (int)ICONSTANT(op2).value);
                }
            }
            break;
        case Minus_expr:
            if (IS_SIGNED_TYPE(type)) {
                if (IS_64BIT_TYPE(type)) {
                    return DIFFERENT_SIGNS(val1, val2) && 
                            DIFFERENT_SIGNS(val1 - val2, val2);
                } else {
...
int va_encoded_state(TreeNode* t) {
    ...
    if (ID_DECL(decls).id == builtins[2]) {
        res = 1;
    } else if (TREE_CODE(TREE_TYPE(decls)) == Double_type &&
               TREE_LINK(decls) != NULL &&
