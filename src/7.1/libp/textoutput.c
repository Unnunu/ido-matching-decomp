...
/* Pascal-called routine to output an unsigned 64 bit integer. */
void
write_card64 (f, i, width, radix)
     register FILE *f;
...
#define BUFLEN 65
    ...
    write_chars (f, k, (unsigned int)n);
    if ((-width) > (unsigned int)n) {
	    pad (f, ' ', (-width) - (unsigned int)n);
    }
#undef BUFLEN
}